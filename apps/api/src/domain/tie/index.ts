...
export async function checkTie(): Promise<TieResult> {
  ...
  for (const a of accounts) {
    ...
    if (stmt !== docs || (docs !== 0n && balance !== 0n && docs - balance > 1_000_000n)) {
      mismatches.push({ account_id: accountId, statement: stmt.toString(), documents: docs.toString(), balance: balance.toString() });
    }
  }
